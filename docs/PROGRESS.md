...
- **유파 인물 3종의 소스 비율이 어긋난다(코드 아님, 에셋 문제).**
  - `style-1`만 가로형 929x713이고 `style-2`는 436x1481, `style-3`은 295x1481 세로형이다.
    코드가 `contain` + 바닥 정렬이라 가로형 하나만 폭에 걸려 다르게 앉는다
  - 실측(1512x900): 인물 높이 336 / 334 / 392px, **폭 454 / 192 / 138px(3.3배 차)**,
    인물 하단 y 862 / 750 / 766px(**style-1이 112px 낮게 앉는다**)
  - 셋을 같은 세로 비율로 다시 잘라 오는 것이 정석이다. 코드로 맞추려면 섹션 소유 세션의 판단이 필요하다
