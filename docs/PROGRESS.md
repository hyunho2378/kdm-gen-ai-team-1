...
- **브랜드/컨셉/유파 임베드 사진 전부 제출 전 자체 제작 교체 필수.**
  - 원본 슬라이드 SVG(`Slide 16_9 - 29 / 30`)에 base64로 박혀 있던 사진은 **라이선스 미확인 시안이라
    코드로 임베드하지 않았다.** CREDITS에 채택 기록 없음
  - 지금 그 자리는 전부 다크 플레이스홀더다. 규약 경로에 파일을 놓으면 코드 수정 0으로 반영된다:
    `images/keyword/dynamic.png` `precision.png` `immersion.png`,
    `images/naming/shot-1.png` `shot-2.png`
