...
- P-C 룰 심화 R1 가드 홀드 전용화, 찌르기 우선 (확인 대기)
  ...
  - **2D 폴백에 같은 분기 순서 버그가 남아 있다.** `canvas2d/fighters.js`의 포즈 선택과
    `canvas2d/geometry.js`의 `swordTip`이 가드를 먼저 보고 `lungeT`를 무시한다.
    R1 사양이 "pose 채널만 고친다"라 손대지 않았다. 1인칭이 기본이고 2D는 폴백이지만
    폴백에서도 되찌르기가 안 읽히므로 **고칠지 여부는 사용자 판단이다**
