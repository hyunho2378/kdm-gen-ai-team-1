...
## 디자인 키워드 슬라이드 인터랙션 (PV3)

- **인트로 둘째 줄 제거.** `copy.js` KEYWORD.body에서 '이를 위해 Dynamic, Precision, Immersion의 세가지...'
  줄을 삭제(카드가 세 키워드를 보여주므로 중복). 그 결과 카드가 위로 올라간다(1440에서 cardTop 200→130).
- **방향키 확장(포커스).** 셸 위임 섹션(App `DELEGATE_IDS`에 keyword 포함)이라 방향키가 하위 스텝을 소비한다.
  포커스 카드는 제자리 1.08배 + 나머지는 0.94배 + blur. 경계(0에서 위 / 2에서 아래)에서만 `handleStep`이
  false를 돌려 셸이 섹션을 옮긴다(셸과 충돌 없음).
- **확장이 그리드 마진을 안 넘게.** 가운데 기준 확대는 1.08배에서 가장자리 카드/캡션이 marginX를 17px 넘었다(실측).
  **가장자리 카드의 `transformOrigin`을 안쪽으로**(첫 카드 left center, 끝 카드 right center) 잡아
  바깥 변을 마진에 고정하고 안쪽으로만 키운다. 세 포커스 상태 모두 카드/캡션이 마진 안(실측 45/1395, 오버플로 0).
