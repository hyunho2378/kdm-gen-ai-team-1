...
| 음영 | **부위 구분이 명확한 유니폼 음영.** 팔과 몸통, 다리와 몸통, 좌우 다리가 각각 갈려 보여야 한다. 뒤쪽 팔다리는 앞쪽보다 어둡게. 완전 단색 채움 금지 |
...
**실루엣만 잘라 봐도 팔 다리 몸통이 갈리는가.**

지금 화면에 서 있는 것은 코드로 그린 임시 실루엣이다. 부위 경계 스트로크와 2톤 음영으로
사람으로 읽히게는 해 두었지만(E2) **근본 해결이 아니다.** 이 사양대로 만든 이미지로 교체하면
부위 구분 문제는 완전히 사라진다.
