...
설계가 정하지 않았거나 실물과 어긋난 지점 넷을 실측으로 확정했다.

### 캘리브레이션은 폰이 소유한다

4절은 "poseChannel 안에서만 적용한다"고 썼지만 **C2가 이미 폰에 구현해 두었다.**
`orientation.js`가 기준 자세의 켤레를 곱해 상대 회전을 보내므로 arena가 `setCalibration`을
또 부르면 두 번 보정되어 검이 엉뚱한 데를 본다. **arena는 받은 쿼터니언을 그대로 쓴다.**

`MSG.CALIB`은 보정값이 아니라 **준비 완료 통지**다. arena가 이걸 받고 CALIBRATION을 벗어난다.
탭 버튼 모드는 캘리브레이션을 건너뛰므로 폰이 그때도 통지를 보낸다(안 보내면 arena가 폰을 기다리다 멈춘다).
싣는 값은 기준 쿼터니언이다. `read()`가 돌려주는 rel은 다음 센서 이벤트에서야 갱신되므로
`setBaseline` 직후에 읽으면 보정 전 값이 나간다(실측으로 92도짜리 엉뚱한 값이 나갔다).

### 연속 채널의 모드는 소스만 바꾼다

`setPreset`이 모드를 `preset`으로 되돌리고 있었다. **GameCanvas가 매 프레임 `setPreset`을 부르므로**
컨트롤러가 쿼터니언을 흘려도 다음 프레임에 프리셋이 덮어써 검이 폰을 전혀 안 따라갔다
(실측: 다섯 자세가 전부 같은 그림). 프리셋 갱신은 값만 바꾸고 모드는 안 건드린다.
쿼터니언에서 내려오는 길은 `fallbackToPreset` 하나뿐이고 부르는 곳은 둘이다. `peer_left`와 F9.

### 회전은 폰이, 위치는 트윈이 쥔다

6절이 말한 우선순위(thrust > guard > 쿼터니언)를 **회전에 그대로 걸면 착탄이 죽는다.**
착탄점은 그립에서 칼끝으로 뻗은 직선을 상대 평면까지 늘린 곳이고 그 직선이 곧 검의 회전이다.
찌르기 순간에 회전을 프리셋으로 되돌리면 겨눈 곳이 아니라 프리셋이 정한 곳에 꽂힌다.
그래서 채널을 갈랐다.

| 채널 | 소유 | 이유 |
|---|---|---|
| 그립 위치 | **언제나 프리셋 트윈** | 찌르기의 뻗음과 가드의 올림이 여기서 나온다. 우선순위는 여기 걸린다 |
| 회전 | **폰이 있으면 폰** | 겨냥은 폰이 아는 것이고 팔이 얼마나 나갔는지는 판정이 안다 |

소스 전환은 0.35초 슬러프다. 붙고 끊길 때 검이 한 프레임에 홱 돌면 사고로 보인다.

### AIM_JITTER는 제거가 아니라 분기다

폰이 검을 쥐는 동안 0이 된다. 조준이 진짜인데 인공 변주를 얹으면 겨눈 곳과 꽂히는 곳이 어긋난다.
키보드 폴백은 여전히 프리셋 트윈이라 변주가 없으면 착탄이 한 점에 박히므로 그대로 산다.
착탄 투영식은 손대지 않았다.
