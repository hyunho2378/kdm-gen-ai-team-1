...
// 시연 안정화(DEMO_STABILIZE): 고DPI 노트북 fps 흔들림을 잡으려 2 → 1.5로 캡을 내렸다.
// 후처리 타깃이 픽셀 수의 제곱으로 비싸져 이 캡이 fps에 가장 크게 듣는다. 시연 노트북에서 확정한다.
const DPR_CAP = 1.5;
 ...
 */
// 시연 안정화(DEMO_STABILIZE): 대각 패리로 스냅할 때 급회전이면 잔상이 크게 번진다.
// 진입을 55 → 100ms로 살짝 늘려 채찍처럼 안 돌게 한다. 여전히 반응으로 읽히고 가드인 게 보인다.
const GUARD_IN_SEC = 0.1;
