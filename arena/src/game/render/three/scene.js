...
export const SWORD_POSES = {
  ...
  // 런지(D3). 전진 홀드 + 찌르기. 한 걸음 더 들어간 뻗기다.
  // 6절 안전 부등식 확인: 그립 ndcY -0.093(54.6%), ndcX 0.048(52.4%)로 여유가 크다
  lungeDeep: { grip: [0.10, -0.12, -1.85], tip: [-0.02, 0.10, -2.30] },
  // 가드(D4 강화). 칼날이 화면 상단을 **가로지르는** 방어선이 되게 눕혔다.
  // 이전 값은 세로로 서 있어 "올린 검"이지 "받아내는 선"으로 읽히지 않았다.
  // 실투영: 그립 (79.2%, 52.7%) → 칼끝 (43.6%, 24.1%). 가로 35.6% 세로 28.6%를 가른다.
  // 6절 안전 부등식: 최대 |ndcX| 0.584(16:10), 4:3에서도 0.700으로 0.92 아래다.
  guard: { grip: [0.34, -0.02, -0.52], tip: [-0.10, 0.30, -0.86] },
};
